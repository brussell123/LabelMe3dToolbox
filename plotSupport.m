function plotSupport(annotation,img,sup)

% Visualize support relations:
Nobjects = length(annotation.object);
parents = find(sup==0);
Nsubplot = length(unique(sup))+2;
...
% $$$ subplot(Ny,Nx,k);
% $$$ showTreeObj(sup,{annotation.object(:).name});
% $$$ axis off;
% $$$ title('Support tree');
