function [F,J] = CostFunctionLM3D_ObjectHeights(params,h,cy,mu,sigma,Hy,f,py,sigma_N)
% Inputs:
...
if length(Hy) > 0
  ...
  F = [F (vy-Hy)/sigma_N];
  J = [J; [zeros(Nv,1) -f*(tan(t)^2+1)/sigma_N*ones(Nv,1)]];
end
...
t13 = t2.^2;
t14 = t13 + 1;
J = [mu.*t4.*t5.*t9 + sigma.*t11.*t12.*t4.*t5;- f.*mu.*t14.*t6.*t9 - f.*sigma.*t11.*t12.*t14.*t6];
